...
function Utbetalingslinje({ idx, utbetalingslinje, onByttMottaker }: UtbetalingslinjeProps) {
    ...
    return (
        <div style={{ border: '1px solid', padding: '1em', fontSize: 12, display: 'flex' }}>
            <div style={{ width: '75%', padding: '0.5em', marginBlockEnd: 0 }}>
                <p style={style}><strong>Totalbeløp:</strong>{utbetalingslinje.totalbeløp}</p>
                <p style={style}><strong>Dagsats:</strong>{utbetalingslinje.dagsats}</p>
                <p style={style}><strong>Grad:</strong>{utbetalingslinje.grad}</p>
                <p style={style}><strong>Stønadsdager:</strong>{utbetalingslinje.stønadsdager}</p>
...
export function Utbetaling({ oppdrag, utbetalingFordeling, setUtbetalingFordeling }: UtbetalingProps) {
    ...
    return (
        <div style={{ border: '1px solid', padding: '1em', marginTop: '1em' }}>
            <p><strong>Fagområde:</strong>{oppdrag.fagområde}</p>
            <p><strong>Mottaker:</strong>{oppdrag.mottaker}</p>
            <p><strong>Nettobeløp:</strong>{oppdrag.nettoBeløp}</p>
